            ...
            {!showReset?(
              <div>
                ...
                <button onClick={doOwnerLogin} style={{width:"100%",padding:"14px",background:busy?"rgba(245,158,11,0.5)":"linear-gradient(135deg,#f59e0b,#f97316)",border:"none",borderRadius:10,fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:15,color:"#030c14",cursor:busy?"not-allowed":"pointer",boxShadow:"0 4px 20px rgba(245,158,11,0.3)",marginBottom:12}}>
                  {busy?"Signing in…":"Enter Command Center &rarr;"}
                </button>
                ...
                {!resetSent?(
                  <div>
                    ...
                    <button onClick={doReset} style={{width:"100%",padding:"13px",background:"rgba(245,158,11,0.15)",border:"1px solid "+amberB,borderRadius:9,fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:14,color:amber,cursor:"pointer",marginBottom:10}}>
                      {busy?"Sending…":"Send Reset Link &rarr;"}
                    </button>
        ...
        {mode==="employee"&&(
          <div style={{background:"rgba(255,255,255,0.98)",borderRadius:16,padding:"28px",boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
            ...
            <button onClick={doEmpLogin} style={{width:"100%",padding:"14px",background:busy?"#a5b4fc":"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",borderRadius:10,fontFamily:"'Nunito',sans-serif",fontWeight:700,fontSize:15,color:"#fff",cursor:busy?"not-allowed":"pointer",boxShadow:"0 8px 28px rgba(99,102,241,0.18)",marginBottom:12}}>
              {busy?"Signing in…":"Go to My Work Hub &rarr;"}
            </button>
      ...
      <div style={{background:"#fff",borderRadius:20,padding:"32px 28px",width:"100%",maxWidth:480,boxShadow:"0 4px 40px rgba(99,102,241,0.12)"}}>
        ...
        {step===3&&(
          <div>
            ...
            <button onClick={()=>{setErr("");setStep(4);}} style={primaryBtn}>Save &amp; Continue &rarr;</button>
        ...
        {step===4&&(
          <div style={{textAlign:"center"}}>
            ...
            <button onClick={finish} disabled={busy} style={{...primaryBtn,opacity:busy?0.7:1,cursor:busy?"not-allowed":"pointer"}}>
              {busy?"Setting up your account…":"Enter My Work Hub &rarr;"}
            </button>
          </div>
        )}
      </div>
      ...
      try{
        ...
        try{
          ...
          if(r.ok){
            ...
            // If API says clocked_out but localStorage says clocked_in &rarr; clear localStorage
            // If API says clocked_in &rarr; ensure localStorage is set
    ...
    <div style={{position:"fixed",inset:0,background:"linear-gradient(135deg,#fff9f0,#fff5e8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}}>
      <div style={{background:"#fff",borderRadius:20,padding:"40px",width:"100%",maxWidth:480,boxShadow:"0 8px 40px rgba(0,0,0,0.12)",animation:"fadeUp 0.4s ease"}}>
        ...
        <button
          ...
          {busy?"Creating location...":"Create Location and Continue &rarr;"}
        </button>
      </div>
    </div>
  );
          ...
          {settingsShowPwChange&&(
            <div style={{background:O.bg3,borderRadius:10,padding:"14px",marginBottom:14}}>
              ...
              <button
                ...
                {settingsPwBusy?"Updating…":"Update Password &rarr;"}
              </button>
            </div>
          )}
        ...
        {inviteDone?(
          ...
          <div>
              ...
              <div style={{marginBottom:12}}>
                ...
                {inviteErr.includes("already")&&(
                  <button onClick={async()=>{
                    ...
                    📧 Resend invite to this email instead &rarr;
                  </button>
                )}
              </div>
            )}
            <button
              ...
              {inviteBusy?"Sending...":"Send Invite &rarr;"}
            </button>
          </div>
        )}
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:400,padding:20}} onClick={()=>{setAddOrgOpen(false);setAddOrgErr("");setAddOrgForm({name:"",type:"Restaurant",address:"",empCount:"1-5"});}}>
          <div style={{background:"#fff",borderRadius:20,padding:"32px 28px",width:"100%",maxWidth:480,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
            ...
            <button disabled={addOrgBusy} onClick={async()=>{
              ...
              {addOrgBusy?"Creating...":"Create Company &rarr;"}
            </button>
          </div>
        </div>
      )}
              ...
              return (
                <div style={{background:"#fff",borderLeft:"4px solid "+O.amber,borderRadius:"0 14px 14px 0",padding:"18px 20px",marginBottom:20,boxShadow:O.shadow}}>
                  ...
                  {todayShifts.length>0?(
                    ...
                    <div style={{padding:"10px 0",marginBottom:12,fontFamily:O.sans,fontSize:13,color:O.textF}}>No shifts scheduled for today. <button onClick={()=>setTab("schedule")} style={{background:"none",border:"none",color:O.amber,fontFamily:O.sans,fontSize:13,cursor:"pointer",textDecoration:"underline"}}>Build schedule &rarr;</button></div>
                  )}
                ...
                <div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:14}}>
                      ...
                      <div style={{background:"#fff",border:"1px solid "+O.border,borderRadius:14,padding:"18px",boxShadow:O.shadow}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                          ...
                          <button onClick={()=>setTab("schedule")} style={{fontFamily:O.sans,fontSize:12,fontWeight:600,color:O.amber,background:O.amberD,border:"1px solid "+O.amberB,borderRadius:6,padding:"4px 10px",cursor:"pointer"}}>View Full &rarr;</button>
                        </div>
                        {liveShifts===null&&(
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:14}}>
                      ...
                      <div style={{background:"#fff",border:"1px solid "+O.border,borderRadius:14,padding:"18px",boxShadow:O.shadow}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                          ...
                          <button onClick={()=>setTab("staff")} style={{fontFamily:O.sans,fontSize:12,fontWeight:600,color:O.amber,background:"none",border:"none",cursor:"pointer"}}>View All &rarr;</button>
                        </div>
                        {LIVE.length===0?(
                          <div style={{textAlign:"center",padding:"20px 0"}}>
                            <div style={{fontSize:32,marginBottom:8}}>👥</div>
                  ...
                  {LIVE.length===0&&(
                    <div style={{background:"linear-gradient(135deg,rgba(124,58,237,0.06),rgba(109,40,217,0.04))",border:"1px solid rgba(124,58,237,0.15)",borderRadius:14,padding:"20px 24px",display:"flex",alignItems:"center",gap:16}}>
                      ...
                      <button onClick={()=>setShowInvite(true)} style={{padding:"10px 20px",background:"linear-gradient(135deg,#7c3aed,#6d28d9)",border:"none",borderRadius:9,fontFamily:O.sans,fontWeight:700,fontSize:13,color:"#fff",cursor:"pointer",flexShrink:0,boxShadow:"0 4px 14px rgba(124,58,237,0.3)"}}>Invite Employee &rarr;</button>
                    </div>
                  )}
                  {LIVE.length>0&&(liveShifts||[]).length===0&&(
                    <div style={{background:"linear-gradient(135deg,rgba(224,123,0,0.06),rgba(201,104,0,0.04))",border:"1px solid "+O.amberB,borderRadius:14,padding:"20px 24px",display:"flex",alignItems:"center",gap:16}}>
                      ...
                      <button onClick={()=>setTab("schedule")} style={{padding:"10px 20px",background:"linear-gradient(135deg,#e07b00,#c96800)",border:"none",borderRadius:9,fontFamily:O.sans,fontWeight:700,fontSize:13,color:"#fff",cursor:"pointer",flexShrink:0,boxShadow:"0 4px 14px rgba(224,123,0,0.3)"}}>Build Schedule &rarr;</button>
                    </div>
                  )}
                </div>
              );
              ...
              <div>
                ...
                <div>
                  ...
                  {timeOffRequests.map(req=>{
                    ...
                    const dateLabel=isSameDay?fmtD(req.start_date):`${fmtD(req.start_date)} &rarr; ${fmtD(req.end_date)}`;
          ...
          <div style={{animation:"fadeUp 0.3s ease",paddingBottom:40}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                ...
                {schedViewMode==="week"&&<>
                  ...
                  <button onClick={()=>{setCurrentWeekOffset(w=>w+1);setLiveShifts(null);setSchedPublished(false);}} style={{padding:"8px 12px",background:"#fff",border:"1px solid "+O.border,borderRadius:8,fontFamily:O.sans,fontSize:13,cursor:"pointer",color:O.textD}}>Next &rarr;</button>
                </>}
            ...
            {liveEmps!==null&&liveEmps.length===0&&(
              <div style={{textAlign:"center",padding:"80px 20px"}}>
                ...
                <button onClick={()=>setTab("staff")} style={{padding:"11px 24px",background:"linear-gradient(135deg,#7c3aed,#6d28d9)",border:"none",borderRadius:9,fontFamily:O.sans,fontWeight:600,fontSize:13,color:"#fff",cursor:"pointer"}}>Go to Staff &rarr;</button>
              </div>
            )}
          ...
          <div style={{animation:"fadeUp 0.3s ease",paddingBottom:40}}>
            ...
            {livePayroll!==null&&livePayroll.length===0&&liveEmps!==null&&liveEmps.length===0&&(
              <div style={{textAlign:"center",padding:"80px 20px"}}>
                ...
                <button onClick={()=>setTab("staff")} style={{padding:"11px 24px",background:O.greenD,border:"1px solid rgba(26,158,110,0.2)",borderRadius:9,fontFamily:O.sans,fontWeight:600,fontSize:13,color:O.green,cursor:"pointer"}}>Go to Staff &rarr;</button>
              </div>
            )}
            ...
            {livePayroll!==null&&livePayroll.length===0&&liveEmps!==null&&liveEmps.length>0&&(
              <div style={{textAlign:"center",padding:"80px 20px"}}>
                ...
                <button onClick={()=>setTab("schedule")} style={{padding:"11px 24px",background:O.amberD,border:"1px solid "+O.amberB,borderRadius:9,fontFamily:O.sans,fontWeight:600,fontSize:13,color:O.amber,cursor:"pointer"}}>Build Schedule &rarr;</button>
              </div>
            )}
            ...
            {pwDone?(
              ...
              <div>
                ...
                <button onClick={handleSetPassword} style={{width:"100%",padding:"14px",background:pwBusy?"rgba(245,158,11,0.5)":"linear-gradient(135deg,#f59e0b,#f97316)",border:"none",borderRadius:10,fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:15,color:"#030c14",cursor:pwBusy?"not-allowed":"pointer",boxShadow:"0 4px 20px rgba(245,158,11,0.3)"}}>
                  {pwBusy?"Setting password…":"Set Password &amp; Sign In &rarr;"}
                </button>
              </div>
            )}
          </div>
